	...
	: public FJsonSerializable
{
	...
	TArray<ULiveLinkExtendPoint3>	NormalList;
	...
	BEGIN_JSON_SERIALIZER
	{
		...
		{
			Serializer.StartArray( "NormalList" );
			for( auto& point : this->NormalList )
			{
	...
	: public FJsonSerializable
{
	...
	TArray<ULiveLinkExtendPoint3>	PointNormalList;
	...
	BEGIN_JSON_SERIALIZER
		...
		{
			Serializer.StartArray( "PointNormalList" );
			for( auto& point : this->PointNormalList )
			{
				point.Serialize( Serializer, bFlatObject );
			}
			Serializer.EndArray();
